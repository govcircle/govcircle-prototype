...
export const AmendmentModal: React.FC<AmendmentModalProps> = ({ isOpen, onClose, onSave }) => {
    ...
    // Data Source
    const constitution = versionHistoryData.find(v => v.meta.version === 2.3) || versionHistoryData[0];
                                ...
                                <div className="flex flex-col gap-4 mb-6">
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        <div className="bg-surface-secondary p-1 rounded-lg flex border border-border">
                                            ...
                                            >
                                                <div className="flex items-center gap-2"><FileText size={16}/> Structure</div>
                                            </button>
                                        </div>
                                    </div>
                                ...
                                <div className={`p-4 rounded-lg mb-6 ${selectedItem || isRootSelected ? 'bg-primary/10' : 'bg-orange-500/10'}`}>
                                    {!selectedItem && !isRootSelected ? (
                                        <div className="flex items-center gap-2 text-orange-400">
                                            ...
                                            <span className="text-sm font-medium">Please select a {actionType === 'Add' ? 'Parent ConstitutionContent' : (targetType === 'Content' ? 'ConstitutionContent' : targetType)} from the Explorer on the left.</span>
                                        </div>
                                    ) : (
                                        <div>
                                            <div className="text-xs text-primary font-bold uppercase mb-1">
                                                {actionType === 'Add' ? `Adding to Parent ${isRootSelected ? 'Root' : (selectedType === 'Content' ? 'ConstitutionContent' : selectedType)}` : `Base ${selectedType === 'Content' ? 'ConstitutionContent' : selectedType}`}
                                            </div>
                                ...
                                <div className={`space-y-4 transition-opacity duration-200 ${(!selectedItem && !isRootSelected) ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                                    ...
                                    {actionType === 'Remove' && (
                                        <div className="p-4 bg-red-500/10 rounded-lg text-red-400 text-sm flex items-start gap-2">
                                            ...
                                            <div>
                                                <strong className="font-bold">Warning:</strong> You are about to remove this {targetType === 'Content' ? 'ConstitutionContent' : targetType}.
                                                {targetType === 'Content' && " Ensure all child rules and ConstitutionContents have been removed first, otherwise this amendment may be invalid."}
                                            </div>
                                        </div>
                                    )}
                                </div>
