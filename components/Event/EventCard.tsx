...
import { Twitter, Youtube, Facebook, Instagram, Monitor, MessageCircle, Ban, ExternalLink, CalendarX, PlayCircle } from 'lucide-react';
...
export const EventCard: React.FC<EventCardProps> = ({ event, onCancel, hideAttendanceButton = false }) => {
    ...
    // Open specific event link from data
    ...
    // Open community space link for main actions
    const handleActionClick = (e: React.MouseEvent) => {
        e.stopPropagation();
        window.open('https://x.com/govcircle_space', '_blank');
    };
        ...
        return (
            <div className="text-textSecondary text-xs font-medium flex items-center flex-wrap gap-1 leading-snug">
                ...
                <span
                    ...
                    className="text-textPrimary capitalize font-bold cursor-pointer hover:text-primary hover:underline transition-all"
                    title={event.link || 'No link provided'}
                >
                {event.socialMedia}
             </span>
                <span
                    onClick={handleFooterLinkClick}
                    className="text-textPrimary cursor-pointer hover:text-primary transition-colors"
                    title={event.link || 'No link provided'}
                >
                {getSocialIcon(event.socialMedia)}
             </span>
                ...
                <div className="flex flex-col gap-2">
                    {showCancelButton ? (
                        ...
                        <button
                            ...
                            className="w-full py-2.5 rounded-lg border border-border font-semibold text-sm transition-all duration-200 hover:bg-surface-secondary hover:border-primary/50 flex items-center justify-center gap-2 text-textPrimary"
                        >
                            {config.buttonLabel}
                    ...
                    {/* Extra Watch Button for Finished Events */}
                    {!hideAttendanceButton && event.status === 'Finished' && (
                        <button
                            onClick={handleActionClick}
                            className="w-full py-2.5 rounded-lg border border-border font-semibold text-sm transition-all duration-200 hover:bg-surface-secondary hover:border-primary/50 flex items-center justify-center gap-2 text-textPrimary"
                        >
